...
const reconnectBtn = document.getElementById('refreshConnection');
reconnectBtn.addEventListener('click', checkFirebaseConnection);
...
function renderQuestionTable(){
  ...
  questions.forEach((q,i)=>{
    ...
    tdQ.addEventListener('click',()=>{ tdQ.classList.toggle('expanded'); });
...
startQuizBtn.addEventListener('click', ()=>{
  ...
  currentQuestionIndex=0; playerScore=0; selectedAnswer=null; startTime=Date.now();
...
confirmAnswerBtn.addEventListener('click', ()=>{
  ...
  if(currentQuestionIndex>=questions.length){
    ...
    quizResult.innerHTML = `${playerName} 得分：${playerScore}，用時 ${((Date.now()-startTime)/1000).toFixed(2)} 秒
      <br><button id="restartQuizBtn" class="btn">重新開始</button>`;
...
function renderLeaderboard(){
  ...
  leaderboard.sort((a,b)=>{
    if(b.score!==a.score) return b.score-b.score;
  ...
  leaderboard.forEach((p,i)=>{
    ...
    tr.innerHTML=`<td class="${cls}">${i+1}</td><td class="expandable">${p.name}</td><td>${p.score}</td><td>${(p.time/1000).toFixed(2)}</td>
      <td><button class="btn danger" data-id="${p.id}">刪除</button></td>`;
    leaderboardTableBody.appendChild(tr);

    const tdName=tr.querySelector('.expandable');
    tdName.addEventListener('click', ()=>{ tdName.classList.toggle('expanded'); });
...
clearLeaderboardBtn.addEventListener('click', async ()=>{
  ...
  if(pwd!==ADMIN_PASSWORD){ alert("管理員密碼錯誤"); return; }
  const snapshot = await db.collection('leaderboard').get();
  for(const doc of snapshot.docs){ await db.collection('leaderboard').doc(doc.id).delete(); }
  fetchLeaderboardFromFirestore();
});
...
// Firebase 實時更新排行榜
db.collection('leaderboard').onSnapshot(fetchLeaderboardFromFirestore);
