...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('importBtn').addEventListener('click', async ()=>{
    ...
    reader.onload = async e=>{
      ...
      for(let i=1;i<lines.length;i++){
        ...
        await addQuestion({q,A,B,C,D,correct});
        addedCount++;
      }
  ...
  document.getElementById('exportCSVBtn').addEventListener('click', async ()=>{
    ...
    const body=data.map(q=>`${q.q},${q.A},${q.B},${q.C},${q.D},${q.correct}`).join('\n');
  ...
  startQuizBtn.addEventListener('click', ()=>{
    ...
    if(!playerName.match(/^[\u4e00-\u9fa5A-Za-z0-9_]{1,16}$/)){alert('名稱不合法'); return;}
  ...
  function renderLeaderboard(){
    ...
    leaderboardTableBody.querySelectorAll('button').forEach(btn=>{
      btn.addEventListener('click', async e=>{
        ...
        leaderboard.splice(parseInt(e.currentTarget.dataset.index),1);
        renderLeaderboard();
      });
    });
  }
  ...
  document.getElementById('addQuestionBtn').addEventListener('click', async ()=>{
    ...
    if(!q||!A||!B||!C||!D||!correct){alert("請填寫完整"); return;}
